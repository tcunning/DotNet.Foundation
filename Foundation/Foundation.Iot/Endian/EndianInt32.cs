...
/// </para>
/// <para>There are also several extension methods, <see cref="EndianInt32Extension"/> that make this "easier" to use for
/// common use cases such as converting values to/from buffers in the desired <see cref="EndianFormat"/>.</para>
/// </summary>
public readonly ref struct EndianInt32 
{
    /// <summary>
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<Int32, TypeSizeOfValue<Int32>>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    public EndianInt32(Int32 value, EndianFormat endianFormat)
    {
    ...
    public EndianInt32(ArraySegment<byte> buffer, EndianFormat endianFormat) : this(MakeValue(buffer, endianFormat), endianFormat)
    {
    }
    ...
    /// <param name="endianFormat">The endian format to use when converting from the buffer to the value</param>
    public static Int32 MakeValue(ArraySegment<byte> buffer, EndianFormat endianFormat)
    {
        if (buffer.Count != EndianValueManipulation<Int32, TypeSizeOfValue<Int32>>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        Int32 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((Int32)buffer[index]) << EndianValueManipulation<Int32, TypeSizeOfValue<Int32>>.BitsToShift(endianFormat, index);
        }
