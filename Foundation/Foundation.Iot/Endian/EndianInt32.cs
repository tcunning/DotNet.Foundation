...
public readonly ref struct EndianInt32 
{
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<Int32>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    {
        if (buffer.Count != EndianValueManipulation<Int32>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        Int32 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((Int32)buffer[index]) << EndianValueManipulation<Int32>.BitsToShift(endianFormat, index);
        }
