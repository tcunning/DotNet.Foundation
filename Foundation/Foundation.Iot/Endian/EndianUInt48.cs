...
/// </para>
/// <para>There are also several extension methods, <see cref="EndianUInt48Extension"/> that make this "easier" to use for
/// common use cases such as converting values to/from buffers in the desired <see cref="EndianFormat"/>.</para>
/// </summary>
public readonly ref struct EndianUInt48
{
    public const UInt64 UInt48Mask = 0x0000FFFF_FFFFFFFF;
    ...
    /// and Given a Value of 0x123456789ABC:
    /// <para/>this[0] == 0xBC
    /// <para/>this[1] == 0x9A
    /// <para/>this[2] == 0x78
    /// <para/>this[3] == 0x56
    ...
    public EndianUInt48(UInt64 value, EndianFormat endianFormat)
    {
        if( value > UInt48Mask )
            throw new ArgumentOutOfRangeException(nameof(value));

        EndianFormat = endianFormat;
        Value = value & UInt48Mask;
    }
    ...
    public EndianUInt48(ArraySegment<byte> buffer, EndianFormat endianFormat) : this(MakeValue(buffer, endianFormat), endianFormat)
    {
    }
    ...
    public EnumeratorForSix<byte> GetEnumerator() => new (this[0], this[1], this[2], this[3], this[4], this[5]);
}