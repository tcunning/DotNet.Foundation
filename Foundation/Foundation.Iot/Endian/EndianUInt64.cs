...
public readonly ref struct EndianUInt64 
{
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<UInt64>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    {
        if (buffer.Count != EndianValueManipulation<UInt64>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        UInt64 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((UInt64)buffer[index]) << EndianValueManipulation<UInt64>.BitsToShift(endianFormat, index);
        }
