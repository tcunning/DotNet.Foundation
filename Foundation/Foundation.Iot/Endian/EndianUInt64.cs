...
/// </para>
/// <para>There are also several extension methods, <see cref="EndianUInt64Extension"/> that make this "easier" to use for
/// common use cases such as converting values to/from buffers in the desired <see cref="EndianFormat"/>.</para>
/// </summary>
public readonly ref struct EndianUInt64 
{
    /// <summary>
    ...
    /// </summary>
    public int Count => sizeof(UInt64);
    ...
    /// and Given a Value of 0x123456789ABCDEF0:
    /// <para/>this[0] == 0xF0
    /// <para/>this[1] == 0xDE
    /// <para/>this[2] == 0xBC
    /// <para/>this[3] == 0x9A
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<UInt64, TypeSizeOfValue<UInt64>>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    public EndianUInt64(UInt64 value, EndianFormat endianFormat)
    {
    ...
    public EndianUInt64(ArraySegment<byte> buffer, EndianFormat endianFormat) : this(MakeValue(buffer, endianFormat), endianFormat)
    {
    }
    ...
    {
        if (buffer.Count != EndianValueManipulation<UInt64, TypeSizeOfValue<UInt64>>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        UInt64 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((UInt64)buffer[index]) << EndianValueManipulation<UInt64, TypeSizeOfValue<UInt64>>.BitsToShift(endianFormat, index);
        }
    ...
    public EnumeratorForEight<byte> GetEnumerator() => new (this[0], this[1], this[2], this[3], this[4], this[5], this[6], this[7]);
}