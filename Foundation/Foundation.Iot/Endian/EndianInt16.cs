...
public readonly ref struct EndianInt16
{
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<Int16, TypeSizeOfValue2Bytes>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    {
        if (buffer.Count != EndianValueManipulation<Int16, TypeSizeOfValue2Bytes>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        Int16 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= (Int16)(buffer[index] << EndianValueManipulation<Int16, TypeSizeOfValue2Bytes>.BitsToShift(endianFormat, index));
        }
