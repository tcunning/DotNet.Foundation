...
/// </para>
/// <para>There are also several extension methods, <see cref="EndianInt16Extension"/> that make this "easier" to use for
/// common use cases such as converting values to/from buffers in the desired <see cref="EndianFormat"/>.</para>
/// </summary>
public readonly ref struct EndianInt16
{
    /// <summary>
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<Int16, TypeSizeOfValue<UInt16>>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    public EndianInt16(Int16 value, EndianFormat endianFormat)
    {
    ...
    public EndianInt16(ArraySegment<byte> buffer, EndianFormat endianFormat) : this(MakeValue(buffer, endianFormat), endianFormat)
    {
    }
    ...
    /// <param name="endianFormat">The endian format to use when converting from the buffer to the value</param>
    public static Int16 MakeValue(ArraySegment<byte> buffer, EndianFormat endianFormat)
    {
        if (buffer.Count != EndianValueManipulation<Int16, TypeSizeOfValue<UInt16>>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        Int16 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= (Int16)(buffer[index] << EndianValueManipulation<Int16, TypeSizeOfValue<UInt16>>.BitsToShift(endianFormat, index));
        }
