...
public readonly ref struct EndianUInt32 
{
    ...
    public byte this[int index] => (byte)((Value >> EndianValueManipulation<UInt32, TypeSizeOfValue4Bytes>.BitsToShift(EndianFormat, index)) & Byte.MaxValue);
    ...
    {
        if (buffer.Count != EndianValueManipulation<UInt32, TypeSizeOfValue4Bytes>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        UInt32 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((UInt32)buffer[index]) << EndianValueManipulation<UInt32, TypeSizeOfValue4Bytes>.BitsToShift(endianFormat, index);
        }
