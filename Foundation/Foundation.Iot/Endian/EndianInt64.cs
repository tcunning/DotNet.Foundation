...
/// </para>
/// <para>There are also several extension methods, <see cref="EndianInt64Extension"/> that make this "easier" to use for
...
public readonly ref struct EndianInt64 
{
    ...
    public EndianInt64(Int64 value, EndianFormat endianFormat)
    {
    ...
    public EndianInt64(ArraySegment<byte> buffer, EndianFormat endianFormat) : this(MakeValue(buffer, endianFormat), endianFormat)
    {
    }
    ...
    {
        if (buffer.Count != EndianValueManipulation<Int64, TypeSizeOfValue8Bytes>.NumberOfBytes)
            throw new ArgumentOutOfRangeException(nameof(buffer), $"Unexpected buffer size");

        Int64 value = 0x00;
        for (var index = 0; index < buffer.Count; index++) {
            value |= ((Int64)buffer[index]) << EndianValueManipulation<Int64, TypeSizeOfValue8Bytes>.BitsToShift(endianFormat, index);
        }
